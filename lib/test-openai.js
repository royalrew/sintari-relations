...
// Ladda .env från backend-mappen först, sedan root
const envPaths = [
...
for (const envPath of envPaths) {
  if (existsSync(envPath)) {
    config({ path: envPath });
    loadedPath = envPath;
    console.log(`📁 Laddade .env från: ${envPath}`);
    break;
  }
}

if (!loadedPath) {
  console.warn("⚠️  Ingen .env-fil hittades. Kontrollera att backend/.env finns.");
}
