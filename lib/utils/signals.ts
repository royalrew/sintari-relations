...
export function extractSignals(description: string): ExtractedSignals {
  ...
  const repair_signals: RepairSignals = hasRepairSignals ? "YES" 
    : (hasWarmth ? "MAYBE" : "NO");
  ...
  // Enhanced logic for obvious violence cases
  ...
  const abuse_mention = /förbannad|hata dig|skit|jävla|helvete/i.test(description);
